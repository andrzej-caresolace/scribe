defmodule SocialScribeWeb.SalesforceModalTest do
  ...
  describe "Salesforce Modal - with credential" do
    setup %{conn: conn} do
      user = user_fixture()
    ...
    test "renders Salesforce modal when navigating to salesforce route", %{
      conn: conn,
      meeting: meeting
    } do
    ...
    test "shows Salesforce Integration on meeting page", %{conn: conn, meeting: meeting} do
  ...
  describe "SalesforceModalComponent events" do
    ...
    test "search input accepts input", %{conn: conn, meeting: meeting} do
      {:ok, view, _html} = live(conn, ~p"/dashboard/meetings/#{meeting.id}/salesforce")
